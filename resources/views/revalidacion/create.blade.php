...
<form action="{{route('revalidacion.store', [$id_predictamen, $uabjo_materia->id])}}" method="post">
	@csrf
        ...
        <div class="form-group">
            ...
            <select name="uabjo_optativa" class="form-control">
                @foreach($uabjo_optativas as $uabjo_optativa)
                    <option value="{{$uabjo_optativa->id}}">{{$uabjo_optativa->nombre}}</option>
                @endforeach
            </select>
        </div>
	@endif
        ...
        <div class="form-group">
        	...
        	<select name="universidad_carrera_materia" id="mod_select" class="form-control">
        		...
        		@foreach($universidad_materias as $universidad_materia)
        			<option value="{{$universidad_materia->id}}" class="{{$universidad_materia->tipo}}">{{$universidad_materia->nombre}}</option>
        		@endforeach
        	</select>
        </div>
        ...
        <div class="form-group">
    	    ...
    	    <input type="number" name="calificacion" id="calificacion" min="6" max="10" step="0.1" value="6.0" class="form-control" />
        </div>
...
$('#mod_select').change(function(){
	...
	@if (count($universidad_optativas) > 0)
		select_opt += "<select name='universidad_carrera_optativa' class='form-control'>";
			@foreach($universidad_optativas as $universidad_optativa)
				select_opt += "<option value='{{$universidad_optativa->id}}'>{{$universidad_optativa->nombre}}</option>";
			@endforeach
		select_opt += "</select>";
	@endif
