...
<p id="seccion_crear">
        ...
        <div class="row">
              <div class="col-md-12">
                <div class="form-group">
                    @if (session()->has('id_universidad'))
                            {!! session()->get('id_universidad') !!}
                    @else
                    <label>Universidades</label> <br> 
        ...
        <div class="row">
              <div class="col-md-12">
                <div class="form-group" id="seccion_universidad_carreras">

                            @if (session()->has('id_universidad_carrera'))
                                        {!! session()->get('id_universidad_carrera') !!}
                            @endif
                </div>
            </div>
        </div>
        ...
        <div class="row">
              <div class="col-md-12">
                <div class="form-group" id="seccion_uabjo_carreras">
                    <!--<select class="form-control" id="uabjo_carrera">
                        <option value="0">...</option>
                        @foreach($uabjo_carreras as $uabjo_carrera)
                            <option value="{{$uabjo_carrera->id}}">{{$uabjo_carrera->nombre}}</option>
                        @endforeach
                    </select>-->

                            @if (session()->has('id_uabjo_carrera'))
                                        {!! session()->get('id_uabjo_carrera') !!}
                            @endif
                </div>
            </div>
        </div>
...
function cargar(){
    ...
    if( ( $("#uabjo_carrera").length) && ( $("#universidades").length) && ( $("#universidad_carrera").length) ){
          
     var id_uabjo_carrera = $("#uabjo_carrera").val();
     var id_universidad = $("#universidades").val();
     var id_universidad_carrera = $("#universidad_carrera").val();

     
    var url = "{{route('alumnos.create', [':id_uabjo_carrera', ':id_universidad', ':id_universidad_carrera'] )}}";
    url = url.replace(':id_uabjo_carrera', id_uabjo_carrera);
    url = url.replace(':id_universidad', id_universidad);
    url = url.replace(':id_universidad_carrera', id_universidad_carrera);

@can('create_alumnos')
        if(id_uabjo_carrera > 0){
            $("#seccion_crear").html("<a href='"+url+"' class='btn btn-primary'><i class='fas fa-plus'></i></a>");
        }else{
            $("#seccion_crear").html("");
        }
@endcan

     $.get("alumnos/registros/"+id_uabjo_carrera+"/"+id_universidad+"/"+id_universidad_carrera, function(data, status){
...
$('#seccion_uabjo_carreras').on('change','#uabjo_carrera',function(){
     //alert( $(this).val() );
     var id_uabjo_carrera = $(this).val();
     var id_universidad = $("#universidades").val();
     var id_universidad_carrera = $("#universidad_carrera").val();
...
$('#universidades').change(function() {
    ...
    if(this.value > 0){
         $.get("alumnos/universidad_carrera/"+id_universidad, function(data, status){
             $('#seccion_universidad_carreras').html(data);
             
         });
    }else{
             $('#seccion_universidad_carreras').html("");
             $('#seccion_uabjo_carreras').html("");
             $("#seccion_crear").html("");
             $('#example1').DataTable().destroy();
             $("tbody").empty();
             $('#example1').DataTable().draw();
...
$('#seccion_universidad_carreras').on('change','#universidad_carrera',function(){
    if(this.value > 0){
        $.get("alumnos/uabjo_carrera", function(data, status){
             $('#seccion_uabjo_carreras').html(data);
        });
    }else{
             $('#seccion_uabjo_carreras').html("");
             $("#seccion_crear").html("");
...
/*$('#universidad_carrera').change(function() {
     alert( $(this).val() );
